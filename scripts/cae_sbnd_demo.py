"""
Minimal inference demo for the trained SBND CAE anomaly detector.

Two things happen, in order:

1. Generation from random noise: random latent codes z ~ N(0, s^2 I) are
   decoded (CAEModel.sample, s = --latent_scale) -> generated.npz,
   generated.png. The CAE has no prior over its latent space, so this is a
   probe of the decoder rather than true generative sampling.
...
(model_flags_CAE_SBND.sh, /scratch/7DayLifetime/gputnam/training-SBND-CAE/iterA).
...
bearer token (refresh with: htgettoken -a htvaultprod.fnal.gov -i sbnd):
    ...
    LD_PRELOAD=/usr/lib64/libXrdPosixPreload.so HDF5_USE_FILE_LOCKING=FALSE \
        python3 scripts/cae_sbnd_demo.py [--model_path ...] [--input ...]
...
MODEL_CONFIG = dict(
    ...
    ae_latent_dim=512,
    kld_weight=1e-4,     # unused by the CAE
    spatial_latent=True,
    ssim_weight=0.0,
    final_activation="tanh",
)
DEFAULT_MODEL = "/scratch/7DayLifetime/gputnam/training-SBND-CAE/iterA/ema_0.9999_612000.pt"
DEFAULT_INPUT = (
...
def main():
    ...
    p.add_argument("--output_dir", default="./results/cae-sbnd-demo")
    ...
    p.add_argument("--latent_scale", type=float, default=1.0, help="std of the random latent codes")
    ...
    with th.no_grad():
        samples = model.sample(args.num_samples, latent_scale=args.latent_scale, device=device, generator=gen).cpu().numpy()
